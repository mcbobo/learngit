...
def hand_max_cards(hand, n=5):
    m = {hand_rank(h): h for h in itertools.combinations(hand, n)}
    mk = max(m)
    return mk, m[mk]


def hand_max(hand, n=5):
    return max(map(hand_rank, itertools.combinations(hand, n)))
